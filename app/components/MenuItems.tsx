...
const MenuItems = ({ navigation }: { navigation: NavigationItem[] }) => {
    const onCliconLogOutClick = async () => {
        await logout();
    };

    const pathname = usePathname();
    const logOutButton = navigation.length ? <Button className="bg-red-200 text-slate-900 hover:bg-red-700 hover:text-white" size="sm" onClick={onCliconLogOutClick}>
        Logout
    </Button> : '';
    return (
